...
export default function SkillsChat({ candidateId, parsedResume, onChatComplete }) {
  ...
  useEffect(() => {
    async function loadInitialQuestions() {
      try {
        setIsLoading(true);
        const generatedQuestions = await generateSkillVerificationQuestions(parsedResume);
  ...
  const generateMockFollowUp = (userMessage, currentQuestion) => {
    ...
    const followUps = {
      ...
      "Performance Optimization": "That's a solid approach. Have you used any specific tools to measure performance improvements?",
      "Adaptability": "Excellent learning strategy. What's the most recent technology you've learned and how did you apply it?"
    };

    // Default follow-up if skill-specific one isn't available
    const defaultFollowUp = "Thank you for sharing that. Could you tell me more about how you applied this in a real project?";
      ...
      if (nextQuestionIndex < questions.length) {
        ...
        if (Math.random() > 0.5) {
          const followUpMessage = {
            role: 'assistant',
            content: generateMockFollowUp(currentMessage, questions[currentQuestionIndex])
          };
        ...
        const finalMessage = { 
          role: 'assistant', 
          content: "Thank you for answering all my questions! I'll analyze your responses and provide feedback to potential recruiters. Best of luck with your job search!" 
        };
    ...
    <div className="flex flex-col h-[600px] w-full bg-gray-50 rounded-lg shadow">
      ...
      <div className="flex-1 p-4 overflow-y-auto">
        {messages.map((message, index) => (
          <div 
            ...
            className={`mb-4 ${
              message.role === 'user' 
                ? 'ml-auto max-w-3/4 bg-blue-500 text-white rounded-lg rounded-tr-none p-3' 
      ...
      >
        ...
        <button
          ...
          className="bg-blue-600 text-white px-4 py-2 rounded-r-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-400 flex items-center"
        >
