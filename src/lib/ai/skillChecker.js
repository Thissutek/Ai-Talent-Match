...
export async function assessCandidateSkills(parsedResume, chatHistory = []) {
  try {
    ...
    const mockAssessment = {
      ...
      overallScore: 7.5,
      summary: "The candidate demonstrates strong frontend development skills, particularly in React and JavaScript. They have good experience with modern web development practices and some backend exposure. Areas for improvement include containerization technologies, cloud services, and automated testing frameworks."
    };
    
    // Generate confidence scores for each skill
    skills.forEach(skill => {
      // Assign random confidence scores between 0.65 and 0.95
      mockAssessment.verifiedSkills[skill] = (Math.random() * 0.3 + 0.65).toFixed(2);
    });
...
export async function generateSkillVerificationQuestions(parsedResume) {
  try {
    ...
    const mockQuestions = [
      {
        id: 1,
        question: "Can you describe a challenging project where you used React, and how you structured the component hierarchy?",
        skillToVerify: "React",
        purpose: "Technical depth assessment"
      },
      {
        id: 2,
        question: "How do you typically handle state management in your frontend applications?",
        skillToVerify: "JavaScript",
        purpose: "Architecture knowledge"
      },
      {
        id: 3,
        question: "Can you explain how you've implemented responsive designs in your previous work?",
        skillToVerify: "CSS",
        purpose: "Practical application"
      },
      {
        id: 4,
        question: "Tell me about a time when you had to optimize a web application for performance. What approaches did you take?",
...
export function calculateCandidateRanking(assessment) {
  ...
  const skillVerificationScore = 
    Object.values(verifiedSkills).reduce((sum, confidence) => sum + parseFloat(confidence), 0) / 
  ...
  const skillGapPenalty = skillGaps.length * 0.5; // Each gap reduces score by 0.5
  ...
  finalScore = Math.min(Math.max(finalScore * 10, 0), 100);
